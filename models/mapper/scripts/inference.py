...
def run(test_opts):
    ...
    if opts.work_in_stylespace:
        dataset = StyleSpaceLatentsDataset(
            latents=[l.cpu() for l in test_latents], opts=opts)
    else:
    ...
    for input_batch in tqdm(dataloader):
        ...
        with torch.no_grad():
            ...
            result_batch = run_on_batch(
                input_cuda, net, opts.couple_outputs, opts.work_in_stylespace)
            toc = time.time()
        ...
        for i in range(opts.test_batch_size):
            ...
            if test_opts.couple_outputs:
                ...
                torchvision.utils.save_image(couple_output, os.path.join(
                    out_path_results, f"{im_path}.jpg"), normalize=True, range=(-1, 1))
            else:
                torchvision.utils.save_image(result_batch[0][i], os.path.join(
                    out_path_results, f"{im_path}.jpg"), normalize=True, range=(-1, 1))
            torch.save(result_batch[1][i].detach().cpu(), os.path.join(
                out_path_results, f"latent_{im_path}.pt"))
...
def run_on_batch(inputs, net, couple_outputs=False, stylespace=False):
    ...
    with torch.no_grad():
        ...
        if couple_outputs:
            x, _ = net.decoder([w], input_is_latent=True, randomize_noise=False,
                               truncation=1, input_is_stylespace=stylespace)
