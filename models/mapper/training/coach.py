...
from models.mapper.datasets.latents_dataset import (
    LatentsDataset,
    StyleSpaceLatentsDataset,
)
...
class Coach:
    ...
    def train(self):
        ...
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                # training case
                w_hat = w + 0.1 * self.net.mapper(w)  # mapper -> w update
                x_hat, w_hat, _ = self.net.decoder(
                    [w_hat],
                    input_is_latent=True,
                    return_latents=True,
                    randomize_noise=False,
                ...
                # images, result_latent(w) = generator(code)
    ...
    def checkpoint_me(self, loss_dict, is_best):
        save_name = (
            "best_model.pt" if is_best else "iteration_{}.pt".format(self.global_step)
        )
    ...
    def configure_datasets(self):
        if self.opts.latents_train_path:
            ...
            for b in range(self.opts.train_dataset_size // self.opts.batch_size):
                with torch.no_grad():
                    _, train_latents_b = self.net.decoder(
                        [train_latents_z[b : b + self.opts.batch_size]],
                        truncation=0.7,
                        truncation_latent=self.net.latent_avg,
                        return_latents=True,
                    )
        ...
        if self.opts.latents_test_path:
            ...
            for b in range(self.opts.test_dataset_size // self.opts.test_batch_size):
                with torch.no_grad():
                    _, test_latents_b = self.net.decoder(
                        [test_latents_z[b : b + self.opts.test_batch_size]],
                        truncation=0.7,
                        truncation_latent=self.net.latent_avg,
                        return_latents=True,
                    )
        ...
        train_dataset_celeba = LatentsDataset(
            latents=train_latents.cpu(), opts=self.opts
        )
        test_dataset_celeba = LatentsDataset(latents=test_latents.cpu(), opts=self.opts)
    ...
    def calc_loss(self, w, x, w_hat, x_hat):
        ...
        if self.opts.clip_lambda > 0:
            loss_clip = self.clip_loss(x_hat, self.opts.description).mean()
    ...
    def log_metrics(self, metrics_dict, prefix):
        for key, value in metrics_dict.items():
            # pass
            print(
                f"step: {self.global_step} \t metric: {prefix}/{key} \t value: {value}"
            )
    ...
    def parse_and_log_images(self, x, x_hat, title, index=None):
        if index is None:
            path = os.path.join(
                self.log_dir, title, f"{str(self.global_step).zfill(5)}.jpg"
            )
        else:
            path = os.path.join(
                self.log_dir,
        ...
        torchvision.utils.save_image(
            torch.cat([x.detach().cpu(), x_hat.detach().cpu()]),
            path,
            normalize=True,
            scale_each=True,
            range=(-1, 1),
