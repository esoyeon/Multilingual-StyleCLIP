...
class Coach:
    ...
    def validate(self):
        ...
        for batch_idx, batch in enumerate(self.test_dataloader):
            ...
            self.parse_and_log_images(
                x, x_hat, title='images_val', index=batch_idx)
    ...
    def configure_datasets(self):
        if self.opts.latents_train_path:
            ...
            train_latents_z = torch.randn(
                self.opts.train_dataset_size, 512).cuda()
            train_latents = []
    ...
    def calc_loss(self, w, x, w_hat, x_hat):
        ...
        if self.opts.clip_lambda > 0:
            loss_clip = self.clip_loss(x_hat).mean()
    ...
    def parse_and_log_images(self, x, x_hat, title, index=None):
        if index is None:
            ...
            path = os.path.join(
                self.log_dir, title, f'{str(self.global_step).zfill(5)}_{str(index).zfill(5)}.jpg')
