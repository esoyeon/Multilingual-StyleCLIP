...
def GetCode(Gs, random_state, num_img, num_once, dataset_name):
    ...
    for i in range(int(num_img/num_once)):
        ...
        if truncation_psi is not None and truncation_cutoff is not None:
            layer_idx = np.arange(src_dlatents.shape[1])[
                np.newaxis, :, np.newaxis]
            ones = np.ones(layer_idx.shape, dtype=np.float32)
            coefs = np.where(layer_idx < truncation_cutoff,
...
def GetImg(Gs, num_img, num_once, dataset_name, save_name='images'):
    ...
    tmp = 'data/npy/'+dataset_name+'/'+save_name
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--code_type', choices=['w', 's', 's_mean_std'], default='w')
    ...
    if not os.path.isfile('data/model/'+dataset_name+'.pkl'):
        url = 'https://nvlabs-fi-cdn.nvidia.com/stylegan2/networks/'
        name = 'stylegan2-'+dataset_name+'-config-f.pkl'
        os.system('wget ' + url+name + '  -P  data/model/')
        os.system('mv data/model/'+name+' data/model/'+dataset_name+'.pkl')

    if not os.path.isdir('data/npy/'+dataset_name):
        os.system('mkdir data/npy/'+dataset_name)
