...
from models.global_directions.dnnlib import tflib
from models.global_directions.utils.visualizer import HtmlPageVisualizer
...
class Manipulator():
    ...
    def GenerateImg(self, codes):
        ...
        out = np.zeros((num_images, step, self.img_size,
    ...
    def MSCode(self, dlatent_tmp, boundary_tmp):
        ...
        step = len(self.alpha)
        dlatent_tmp1 = [tmp.reshape((self.num_images, -1))
                        for tmp in dlatent_tmp]
        dlatent_tmp2 = [np.tile(tmp[:, None], (1, step, 1))
                        for tmp in dlatent_tmp1]  # (10, 7, 512)
    ...
    def EditOne(self, bname, dlatent_tmp=None):
        if dlatent_tmp == None:
            dlatent_tmp = [
                tmp[self.img_index:(self.img_index+self.num_images)] for tmp in self.dlatents]
